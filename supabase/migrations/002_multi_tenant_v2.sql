-- ============================================
-- MIGRATION: Multi-tenant + Dark Mode
-- Gestor de Ponto v2.0
...
CREATE TABLE IF NOT EXISTS public.empresas (
    ...
    logo_url TEXT,
    cores JSONB DEFAULT '{"primary": "#22c55e", "secondary": "#334155"}'::jsonb,
...
CREATE POLICY "Allow all" ON public.empresas FOR ALL USING (true);

-- 3. ADD empresa_id TO ALL TABLES
...
CREATE POLICY "Allow all" ON public.orgaos FOR ALL USING (true);

DROP POLICY IF EXISTS "Permitir inserção de unidades" ON public.unidades_trabalho;
CREATE POLICY "Allow all" ON public.unidades_trabalho FOR ALL USING (true);

DROP POLICY IF EXISTS "Permitir inserção de lotações" ON public.lotacoes;
CREATE POLICY "Allow all" ON public.lotacoes FOR ALL USING (true);
...
CREATE POLICY "Allow all" ON public.registros_ponto FOR ALL USING (true);

-- 5. SEED DATA: Create default empresa (PROCON)
INSERT INTO public.empresas (id, nome, nome_fantasia, cnpj, email, plano, max_colaboradores, ativo)
VALUES (
